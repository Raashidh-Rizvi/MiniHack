  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`p-3 rounded-2xl text-left transition-all flex flex-col items-start justify-between cursor-pointer ${
                  isSelected
                    ? 'liquid-pill-active shadow-[0_4px_16px_rgba(239,68,68,0.4)]'
                    : 'liquid-btn-glass text-slate-700 dark:text-slate-300'
                }`}
              >
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-xs font-black ${isSelected ? 'text-white' : 'text-slate-800 dark:text-slate-200'}`}>
                    {s.label}
                  </span>
                  <div className={`w-2 h-2 rounded-full ${isSelected ? 'bg-white' : 'bg-slate-300 dark:bg-slate-700'}`} />
                </div>
                <p className={`text-[11px] leading-tight ${isSelected ? 'text-white/85' : 'text-slate-500 dark:text-slate-400'}`}>{s.desc}</p>
              </button>
            );
          })}
        </div>
      ...
      <button
        ...
        className="w-full py-4 px-6 rounded-full text-sm font-bold liquid-btn-crimson flex items-center justify-center space-x-2 active:scale-98 disabled:opacity-50 cursor-pointer"
      >
        {isSubmitting ? (
          <>
      ...
      <div className="pt-2 text-center">
        <button
          ...
          className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-xl text-xs text-red-600 dark:text-red-400 font-bold liquid-btn-glass hover:border-red-500/40 transition-colors cursor-pointer"
        >
